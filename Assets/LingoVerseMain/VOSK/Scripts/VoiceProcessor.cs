...
public class VoiceProcessor : MonoBehaviour
{
    ...
    /// <summary>
    /// Event when voice was no longer detected
    /// </summary>
    ...
    /// </summary>
    public event Action<string> OnRecordingStart;
    ...
    private float _minimumSpeakingSampleValue = 0.005f;

    [SerializeField, Tooltip("The minimum volume to detect voice input for"), Range(0.0f, 1.0f)]
    private float _backgroundMinSampleValue = 0.1f;

    private bool _backgroundRecording = true;
    private float _activeMinSampleValue = 0.0f;
    ...
    private void Awake() {
        _activeMinSampleValue = _backgroundMinSampleValue;
        UpdateDevices();
    }
    ...
    /// <summary>
    /// Activate the voice detection
    /// </summary>
    public void ToggleVoiceMinVolume() {
        _backgroundRecording = !_backgroundRecording;
        _activeMinSampleValue = _backgroundRecording ? _backgroundMinSampleValue : _minimumSpeakingSampleValue;
    }
    ...
    IEnumerator RecordData()
    {
        ...
        if (OnRecordingStart != null)
            OnRecordingStart.Invoke(CurrentDeviceName);
        ...
        while (IsRecording)
        {
            ...
            else
            {
                ...
                if (maxVolume >= _activeMinSampleValue)
                {
                    _transmit= _audioDetected = true;
